...
export const WEAPON_BOW = {
   ...
   *
   * **현재 값은 'nearest'이고, 이건 잠정이다.** playtest 1회차에서 키클롭스 시련이 200판 중
   * 보스 처치 0건(보스 피해량 중앙값 0.0%)이었다. 원인은 grunt/brute가 거의 항상 보스보다
   * 가까워 화살이 영원히 보스에 닿지 않는 것이고, 이 시련은 `wBossKill:1` 단독 조건이라
   * 클리어율이 구조적으로 0%다. 해결안은 GDD §3-A(= 'bossFirstThenNearest')로 확정했다(D28).
   *
   * **전환 조건(기계적)**: DAMAGE 6→10만 반영한 2회차-A 측정이 기록으로 남는 순간 이 값을
   * 'bossFirstThenNearest'로 바꾼다. 두 변경을 같은 측정에 섞으면 무엇이 효과였는지 알 수 없다.
   * tech는 **두 값 모두 동작하도록** 구현한다(플립 한 줄로 A/B가 되게).
   */
  AIM: 'nearest' as AimRule,
} as const;
...
export const ENEMIES = {
  ...
  boss: {
     ...
     * §3-A 반영 후에도 키클롭스 클리어율이 0.35 미만이면 **다음으로 조정할 단 하나의 수치가 이 값**이며,
     * 조정치는 측정된 "보스 조준 유지율"에서 산출한다(감으로 바꾸지 않는다. GDD §6-A).
     */
    HP: 700,
...
export const XP_CURVE = {
  ...
  /**
   * 레벨당 요구량 배율.
   * **누적 실계산(반올림 포함)**: 8레벨업 99 / 11레벨업 213 / 12레벨업 271 XP.
   * (기존 주석의 "레벨13까지 213"은 off-by-one이었다 — 213은 11레벨업(=레벨 12 도달) 지점이다.
   *  값은 바꾸지 않고 서술만 고쳤다. 2026-08-08 gd)
   * §6 합격 구간 8~14레벨업 = 판당 누적 **99~435 XP** 획득을 뜻한다(14레벨업 = 435).
   * 1회차 실측 스폰 XP 총량은 20.7~31.1(회수 효율 45~61% 적용 전)이라 화력 축(DAMAGE)이
   * 먼저 풀리지 않으면 이 곡선을 손대도 의미가 없다 — 그래서 이번 회차에 GROWTH는 건드리지 않는다.
   */
  GROWTH: 1.25,
} as const;
